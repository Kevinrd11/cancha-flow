...
export async function PATCH(request: Request) {
  ...
  const { data: court } = await auth.supabase.from("fields").select("id").eq("business_id", auth.businessId).order("created_at").limit(1).single();
  if (!court) return NextResponse.json({ error: "No hay una cancha configurada" }, { status: 404 });
