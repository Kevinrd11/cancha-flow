...
import { ArrowLeft, ArrowRight, Building2, Check, CircleDollarSign, Clock3, LoaderCircle, MapPin, PartyPopper, UserRound } from "lucide-react";
...
export function OnboardingFlow() {
  ...
  async function submit() {
    ...
    try {
      ...
      setPublicUrl(payload.publicUrl ?? ""); setDone(true);
    } catch (requestError) { setError(requestError instanceof Error ? requestError.message : "Ocurrió un error"); }
    finally { setLoading(false); }
  }

  if (done) return <div className="mx-auto max-w-2xl px-4 py-16 text-center sm:px-6 sm:py-24"><span className="mx-auto grid size-20 place-items-center rounded-3xl bg-mint text-navy"><PartyPopper size={36} /></span><p className="mt-8 section-kicker">Configuración terminada</p><h1 className="mt-3 text-4xl font-bold tracking-[-.04em] text-navy sm:text-5xl">Su cancha está lista.</h1><p className="mx-auto mt-4 max-w-lg text-lg leading-8 text-slate-600">Su cuenta quedó activa de inmediato. Ya puede ingresar con el correo y la contraseña que registró para administrar la agenda.</p>{publicUrl && <div className="mt-8 rounded-2xl border border-line bg-white p-5 text-left"><p className="text-xs font-bold uppercase tracking-wider text-slate-400">Página pública</p><p className="mt-2 truncate font-bold text-green">{publicUrl}</p></div>}<div className="mt-7 grid gap-3 sm:grid-cols-2"><a href="/admin/login" className="inline-flex min-h-13 items-center justify-center rounded-xl bg-green px-5 font-bold text-white">Ingresar al panel</a>{publicUrl && <a href={publicUrl} className="inline-flex min-h-13 items-center justify-center rounded-xl border border-line bg-white px-5 font-bold text-navy">Ver publicación</a>}</div></div>;
  ...
  return <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14"><div className="grid gap-10 lg:grid-cols-[320px_1fr]">
    ...
    <section className="rounded-3xl border border-line bg-white p-5 shadow-[0_18px_60px_rgba(15,35,28,.07)] sm:p-8 lg:p-10">
      {step === 0 && <Step title="Cree su acceso" description="Su cuenta quedará activa al finalizar; no necesita confirmar el correo." icon={UserRound}><div className="grid gap-5"><Input label="Nombre completo" autoComplete="name" value={form.ownerName} onChange={(value) => update("ownerName", value)} placeholder="Mariana Rodríguez" /><Input label="Correo electrónico" type="email" autoComplete="email" value={form.email} onChange={(value) => update("email", value)} placeholder="mariana@micentro.com" /><Input label="Contraseña" type="password" autoComplete="new-password" value={form.password} onChange={(value) => update("password", value)} placeholder="12+ caracteres, mayúscula, número y símbolo" /></div></Step>}
