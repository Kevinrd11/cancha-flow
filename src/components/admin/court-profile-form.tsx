...
import { Check, ImageIcon, LoaderCircle, Save } from "lucide-react";
import { Button } from "@/components/ui/button";
import type { CourtListing } from "@/lib/courts-data";

export function CourtProfileForm({ court }: { court: CourtListing }) {
  const [form, setForm] = useState({ fieldId: court.id, name: court.name, description: court.description, location: court.address, hourlyRate: court.hourlyRate, imageUrl: court.images[0], active: court.active });
  ...
  return <form onSubmit={submit} className="mt-7 grid gap-5 border-t border-line pt-7">
    ...
    <Input label="URL de fotografía principal" type="url" value={form.imageUrl} onChange={(value) => setForm({ ...form, imageUrl: value })} icon={<ImageIcon size={16} />} />
...
function Input({ label, value, onChange, type = "text", icon }: { label: string; value: string; onChange: (value: string) => void; type?: string; icon?: React.ReactNode }) { return <label className="grid gap-2 text-sm font-bold text-navy">{label}<span className="flex min-h-12 items-center gap-2 rounded-xl border border-line px-3">{icon}<input required type={type} value={value} onChange={(event) => onChange(event.target.value)} className="min-w-0 flex-1 font-normal outline-none" /></span></label>; }
