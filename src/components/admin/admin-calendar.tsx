...
export function AdminCalendar({ initialReservations, settings }: { initialReservations: Reservation[]; settings: BusinessSettings }) {
    ...
    <main>
      ...
      <div className="p-4 sm:p-7 lg:p-10">
        <div className="rounded-3xl border border-line bg-white shadow-[0_8px_30px_rgba(16,32,25,.04)]">
          ...
          {view === "day" && <DayView date={format(cursor, "yyyy-MM-dd")} reservations={visibleReservations} onSelect={openDetail} />}
          {view === "week" && <WeekView cursor={cursor} reservations={visibleReservations} onSelect={openDetail} />}
          {view === "month" && <MonthView cursor={cursor} reservations={visibleReservations} onSelect={openDetail} />}
        </div>
      ...
      {modal === "block" && <BlockModal fieldId={settings.fieldId} onClose={() => setModal(null)} />}
...
function BlockModal({ fieldId, onClose }: { fieldId: string; onClose: () => void }) {
  ...
  async function submit(event: React.FormEvent) { event.preventDefault(); setState("Guardando…"); const response = await fetch("/api/admin/blocked-slots", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...form, fieldId }) }); const payload = await response.json(); if (!response.ok) { setState(payload.error); return; } setState("Horario bloqueado correctamente"); }
