...
export async function POST(request: Request) {
  ...
  if (error || !data.user || !data.user.email_confirmed_at) {
    ...
    await recordSecurityEvent({ action: "auth.login", outcome: "failure", requestFingerprint });
  ...
  if (!profile?.active) {
    await supabase.auth.signOut({ scope: "local" });
    await recordSecurityEvent({ action: "auth.login", outcome: "failure", actorId: data.user.id, requestFingerprint });
    if (profile?.role === "owner") {
      return NextResponse.json(
        { error: "Tu cuenta todavía no tiene acceso. La solicitud puede estar pendiente de aprobación." },
        { status: 403 },
      );
    }
    return NextResponse.json({ error: INVALID_CREDENTIALS }, { status: 401 });
