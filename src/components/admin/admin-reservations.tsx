...
const filterStatuses = ["all", "awaiting_payment", "awaiting_approval", "confirmed", "completed", "cancelled", "no_show"];
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex justify-end bg-black/40" role="presentation">
          <section role="dialog" aria-modal="true" aria-label={`Detalle ${selected.reservationCode}`} className="h-full w-full max-w-md overflow-y-auto bg-white p-6 shadow-2xl">
            ...
            <div className="mt-6"><p className="text-sm font-bold">Pago</p><div className="mt-2 flex items-center justify-between rounded-xl border border-line p-4"><div><p className="font-semibold">Comprobante SINPE</p><p className="text-xs text-muted">Estado: {STATUS_LABELS[selected.paymentStatus] ?? selected.paymentStatus}</p></div><strong>{formatCurrency(selected.total)}</strong></div>{selected.paymentStatus === "pending" && <div className="mt-3 grid grid-cols-2 gap-2"><Button variant="secondary" disabled={loading} onClick={() => update({ id: selected.id, paymentStatus: "rejected", status: "awaiting_payment" })}>Rechazar</Button><Button disabled={loading} onClick={() => update({ id: selected.id, paymentStatus: "approved", status: "confirmed" })}><Check size={17} /> Aprobar pago</Button></div>}</div>
            ...
            <div className="mt-6"><p className="text-sm font-bold">Cambiar estado</p><div className="mt-2 grid grid-cols-2 gap-2">{(["confirmed", "completed", "cancelled", "no_show"] as ReservationStatus[]).map((item) => <button key={item} onClick={() => { if (item !== "cancelled" || window.confirm("¿Confirmas que deseas cancelar esta reserva?")) update({ id: selected.id, status: item }); }} className={cn("min-h-11 rounded-xl border border-line px-3 text-sm font-bold", selected.status === item && "border-forest bg-forest text-white")}>{STATUS_LABELS[item]}</button>)}</div></div>
