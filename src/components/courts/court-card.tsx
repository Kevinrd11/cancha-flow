...
export function CourtCard({ court, priority = false }: { court: CourtListing; priority?: boolean }) {
  const publicPath = court.publicPath ?? `/canchas/${court.slug}`;
  return (
    <article className="group overflow-hidden rounded-3xl border border-line bg-white transition hover:-translate-y-1 hover:shadow-[0_18px_55px_rgba(13,35,29,.12)]">
      <Link href={publicPath} className="relative block aspect-[16/10] overflow-hidden bg-forest">
      ...
      <div className="p-5 sm:p-6">
        ...
        <div className="mt-5 flex items-center justify-between border-t border-line pt-4"><p className="flex items-center gap-2 text-sm text-slate-600"><Clock3 size={16} className="text-green" /><span><strong className="text-navy">Próximo:</strong> {court.nextAvailable}</span></p><Link href={`${publicPath}#reservar`} className="inline-flex min-h-10 items-center gap-1 rounded-xl bg-navy px-3 text-sm font-bold text-white">Ver horarios <ArrowRight size={15} /></Link></div>
      </div>
    </article>
  );
}
