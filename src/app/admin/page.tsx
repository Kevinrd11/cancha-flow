...
export default async function AdminDashboard() {
  const [reservations, settings, courts] = await Promise.all([getAdminReservations(), getBusinessSettings(), getBusinessCourts()]);
  ...
  // Las horas libres se cuentan sobre todas las canchas activas: con dos
  // canchas abiertas de 8 a 22 hay 28 horas vendibles al día, no 14.
  const capacityMinutes = activeCourts.reduce((sum, court) => sum + Math.max(0, toMinutes(court.closingTime) - toMinutes(court.openingTime)), 0);
  ...
  return <main>
    ...
    <div className="p-4 sm:p-7 lg:p-9">
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <Metric icon={Clock3} label="Horas libres hoy" value={`${freeHours % 1 ? freeHours.toFixed(1) : freeHours} h`} helper={multiCourt ? `${activeCourts.length} canchas activas` : `${settings.openingTime} – ${settings.closingTime}`} />
        {finance
