...
export function CourtProfileForm({ court, location }: { court: Court; location: string }) {
  ...
  return <form onSubmit={submit} className="mt-7 grid gap-5 border-t border-line pt-7">
    ...
    <Input label="URL de fotografía principal (opcional)" type="url" required={false} value={form.imageUrl} onChange={(value) => setForm({ ...form, imageUrl: value })} icon={<ImageIcon size={16} />} />
