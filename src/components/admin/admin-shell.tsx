...
import { CalendarClock, CalendarRange, LayoutDashboard, LogOut, Menu, Settings, Shapes, ShieldCheck, TicketCheck, Wallet, X } from "lucide-react";
import { useState } from "react";
import { cn } from "@/lib/utils";

// `owner` marca las secciones que el personal operativo no debe ver siquiera
// listadas. El proxy y cada ruta vuelven a comprobarlo en el servidor.
const links = [
  ...
  { href: "/admin/finanzas", label: "Finanzas", icon: Wallet, owner: true },
  { href: "/admin/canchas", label: "Canchas", icon: Shapes, owner: true },
  { href: "/admin/configuracion", label: "Configuración", icon: Settings, owner: true },
  { href: "/admin/seguridad", label: "Seguridad", icon: ShieldCheck },
];

export function AdminShell({ children, role = "owner" }: { children: React.ReactNode; role?: "owner" | "staff" }) {
  const pathname = usePathname(); const [open, setOpen] = useState(false);
  const visible = links.filter((link) => !link.owner || role === "owner");
  ...
  return <div className="min-h-screen bg-[#f4f6f4] lg:grid lg:grid-cols-[248px_1fr]">
    ...
    <aside className={cn("fixed inset-y-0 left-0 z-40 flex w-[248px] -translate-x-full flex-col overflow-y-auto bg-navy p-3 text-white transition lg:sticky lg:top-0 lg:h-screen lg:translate-x-0", open && "translate-x-0")}>
      <Link href="/admin" className="flex h-16 items-center gap-3 px-2" onClick={() => setOpen(false)}><span className="grid size-10 place-items-center rounded-xl bg-mint font-bold text-navy">CF</span><div><strong className="block text-lg tracking-tight">CanchaFlow</strong><span className="text-xs text-white/40">Panel del centro</span></div></Link>
      <nav className="mt-5 space-y-1" aria-label="Panel de propietario">{visible.map(({ href, label, icon: Icon }) => { const active = href === "/admin" ? pathname === href : pathname.startsWith(href); return <Link key={href} href={href} onClick={() => setOpen(false)} className={cn("flex min-h-11 items-center gap-3 rounded-lg px-3 text-sm font-semibold text-white/55 transition hover:bg-white/7 hover:text-white", active && "bg-white/10 text-white")}><Icon size={18} className={active ? "text-mint" : ""} />{label}</Link>; })}</nav>
      <div className="mt-auto pt-5"><div className="mb-2 flex items-center gap-3 rounded-xl border border-white/8 p-3"><span className="grid size-8 place-items-center rounded-full bg-green text-xs font-bold">CF</span><div className="min-w-0"><p className="truncate text-sm font-bold">Cuenta administrativa</p><p className="text-xs text-white/35">Sesión protegida</p></div></div><button onClick={logout} className="flex min-h-10 w-full items-center gap-3 rounded-lg px-3 text-sm font-semibold text-white/45 hover:bg-white/7 hover:text-white"><LogOut size={17} />Cerrar sesión</button></div>
