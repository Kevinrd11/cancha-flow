...
import type { BlockedSlot, BusinessCourtOption, BusinessSettings, Reservation } from "@/lib/types";
...
export function AdminCalendar({ initialReservations, initialBlockedSlots, settings, courts }: { initialReservations: Reservation[]; initialBlockedSlots: BlockedSlot[]; settings: BusinessSettings; courts: BusinessCourtOption[] }) {
  ...
  // El filtro va por id y no por nombre: dos canchas pueden llamarse parecido y
  // el id es lo que enlaza reservas y bloqueos con la cancha.
  const [court, setCourt] = useState("all");
  const visibleReservations = court === "all" ? reservations : reservations.filter((item) => item.courtId === court);
  const visibleBlocks = court === "all" ? blockedSlots : blockedSlots.filter((item) => item.fieldId === court);
  // Con el filtro en una cancha concreta, los formularios la proponen ya
  // elegida: es la que el propietario está mirando.
  const defaultCourtId = court === "all" ? (courts[0]?.id ?? settings.fieldId) : court;
  const multiCourt = courts.length > 1;
  // Viendo todas las canchas a la vez hay que decir de cuál es cada reserva;
  // filtrando por una sola, el rótulo sobra.
  const showCourt = multiCourt && court === "all";
  ...
  return (
    <main>
      ...
      <div className="p-4 sm:p-7 lg:p-10">
        <div className="rounded-3xl border border-line bg-white shadow-[0_8px_30px_rgba(16,32,25,.04)]">
          <div className="flex flex-col gap-4 border-b border-line p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5">
            ...
            <div className="flex flex-wrap gap-2">{multiCourt && <select value={court} onChange={(event) => setCourt(event.target.value)} aria-label="Filtrar por cancha" className="min-h-10 rounded-xl border border-line bg-white px-3 text-sm font-semibold"><option value="all">Todas las canchas</option>{courts.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>}<div className="grid grid-cols-3 rounded-xl bg-paper p-1">{(["day", "week", "month"] as View[]).map((item) => <button key={item} onClick={() => setView(item)} className={cn("min-h-9 rounded-lg px-3 text-sm font-bold", view === item && "bg-white shadow-sm")} aria-pressed={view === item}>{{ day: "Día", week: "Semana", month: "Mes" }[item]}</button>)}</div></div>
          </div>
          {view === "day" && <DayView date={format(cursor, "yyyy-MM-dd")} reservations={visibleReservations} blocks={visibleBlocks} showCourt={showCourt} onSelect={openDetail} onRemoveBlock={removeBlock} />}
          {view === "week" && <WeekView cursor={cursor} reservations={visibleReservations} blocks={visibleBlocks} showCourt={showCourt} onSelect={openDetail} onRemoveBlock={removeBlock} />}
      ...
      {modal === "reservation" && <ReservationModal settings={settings} courts={courts} defaultCourtId={defaultCourtId} onClose={() => setModal(null)} onCreated={(reservation) => { setReservations((items) => [...items, reservation]); setModal(null); }} />}
      {modal === "block" && <BlockModal courts={courts} defaultCourtId={defaultCourtId} onClose={() => setModal(null)} onCreated={(block) => { setBlockedSlots((items) => [...items, block]); setModal(null); }} />}
...
function DayView({ date, reservations, blocks, showCourt, onSelect, onRemoveBlock }: { date: string; reservations: Reservation[]; blocks: BlockedSlot[]; showCourt: boolean; onSelect: (item: Reservation) => void; onRemoveBlock: (item: BlockedSlot) => void }) {
  ...
  return <div className="min-h-[460px] p-4 sm:p-6">{items.length || dayBlocks.length ? <div className="space-y-3">{items.map((item) => <ReservationCard key={item.id} reservation={item} showCourt={showCourt} onClick={() => onSelect(item)} />)}{dayBlocks.map((item) => <BlockCard key={item.id} block={item} onRemove={() => onRemoveBlock(item)} />)}</div> : <EmptyDay />}</div>;
}

function WeekView({ cursor, reservations, blocks, showCourt, onSelect, onRemoveBlock }: { cursor: Date; reservations: Reservation[]; blocks: BlockedSlot[]; showCourt: boolean; onSelect: (item: Reservation) => void; onRemoveBlock: (item: BlockedSlot) => void }) {
  const start = startOfWeek(cursor, { weekStartsOn: 1 });
  const days = Array.from({ length: 7 }, (_, index) => addDays(start, index));
  return <div className="scrollbar-none overflow-x-auto"><div className="grid min-w-[920px] grid-cols-7 divide-x divide-line">{days.map((day) => { const date = format(day, "yyyy-MM-dd"); const items = reservations.filter((item) => item.date === date); const dayBlocks = blocks.filter((item) => item.date === date); return <div key={date} className="min-h-[500px] p-2"><div className={cn("mb-3 rounded-xl p-2 text-center", date === todayInCostaRica() && "bg-lime")}><p className="text-xs font-bold uppercase text-muted">{format(day, "EEE", { locale: es })}</p><p className="display text-2xl font-black">{format(day, "d")}</p></div><div className="space-y-2">{items.map((item) => <ReservationCard compact key={item.id} reservation={item} showCourt={showCourt} onClick={() => onSelect(item)} />)}{dayBlocks.map((item) => <BlockCard compact key={item.id} block={item} showCourt={showCourt} onRemove={() => onRemoveBlock(item)} />)}</div></div>; })}</div></div>;
}
...
function ReservationCard({ reservation, compact, showCourt, onClick }: { reservation: Reservation; compact?: boolean; showCourt?: boolean; onClick: () => void }) {
  return <button onClick={onClick} className={cn("w-full rounded-xl border-l-4 bg-paper p-3 text-left transition hover:bg-lime/15", reservation.status === "confirmed" ? "border-emerald-500" : reservation.status === "cancelled" ? "border-rose-500 opacity-60" : "border-amber-400")}><p className="font-bold">{formatTime(reservation.startTime)}</p><p className="truncate text-sm">{reservation.customerName}</p>{showCourt && <p className="truncate text-xs font-semibold text-green">{reservation.courtName}</p>}{!compact && <p className="mt-1 text-xs text-muted">{reservation.reservationCode} · {formatCurrency(reservation.total)}</p>}</button>;
}

function BlockCard({ block, compact, showCourt, onRemove }: { block: BlockedSlot; compact?: boolean; showCourt?: boolean; onRemove: () => void }) {
  return <div className="w-full rounded-xl border-l-4 border-slate-500 bg-slate-100 p-3 text-left">
    ...
    {(!compact || showCourt) && <p className="mt-1 truncate text-xs font-semibold uppercase tracking-wider text-muted">Bloqueo · {block.courtName}</p>}
  </div>;
}
...
function CourtSelect({ courts, value, onChange }: { courts: BusinessCourtOption[]; value: string; onChange: (value: string) => void }) {
  // Con una sola cancha el selector no aporta nada: se envía igual en el
  // formulario, pero no se muestra.
  if (courts.length < 2) return null;
  return <label className="grid gap-1.5 text-sm font-bold">Cancha<select value={value} onChange={(event) => onChange(event.target.value)} className="min-h-12 min-w-0 rounded-xl border border-line px-3">{courts.map((court) => <option key={court.id} value={court.id}>{court.name}{court.active ? "" : " (inactiva)"}</option>)}</select></label>;
}
