...
const GENERIC_MESSAGE = "Si la cuenta existe, recibirás un enlace para restablecer la contraseña.";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: "El servicio de recuperación no está disponible" }, { status: 503 });
  }
  if (!rateLimit.allowed) {
    await recordSecurityEvent({ action: "auth.recovery_requested", outcome: "blocked", requestFingerprint: rateLimit.fingerprint });
  ...
  const { error } = await supabase.auth.resetPasswordForEmail(parsed.data.email, {
    redirectTo: `${getAppUrl()}/auth/callback?next=/restablecer-contrasena`,
  });
  await recordSecurityEvent({ action: "auth.recovery_requested", outcome: error ? "failure" : "success", requestFingerprint: rateLimit.fingerprint });
